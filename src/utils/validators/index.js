export const solver = (grid, row = 0, col = 0) => {
  ...
  for (let num = 1; num <= 9; num++) {
    if (checkValid(grid, row, col, num)) grid[row][col] = num;
    const [newRol, newCol] = getNext(row, col);

    if (newRol && !newCol) return true;
    if (solver(grid, newRol, newCol)) return true;
  }
  grid[row][col] = -1;
  return false;
};
...
export const compareSudoku = (currentSudoku, solverSudoku) => {
  const res = {
    isComplete: true,
    isSolvable: true,
  };
